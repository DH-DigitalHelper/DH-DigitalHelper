...
def insert_legacy(conn, url, text, content_sha256, now=NOW1, present=1):
    """Insert a document row with text_sha256 left NULL, mimicking a row that predates the dedup column."""
    conn.execute(
...
def test_backfill_streams_over_multiple_batches(tmp_path):
    ...
    result = st.run_dedup(conn, batch_size=2)
...
def test_dedup_keeps_cleanest_across_present_only(tmp_path):
    """A removed (present=0) sibling is never chosen as canonical, and present=0 tombstones are left untouched by the delete."""
    conn = db(tmp_path)
